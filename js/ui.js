...
function updateTodayDateDisplay() {
        ...
        const wrap = document.getElementById('today-date-wrap');
        if (wrap) {
            wrap.setAttribute('data-tooltip', fullStr);
            wrap.setAttribute('title', fullStr);
        }
    }
        ...
        const legacyBtn = document.getElementById('theme-btn');
        if (legacyBtn) {
            legacyBtn.innerHTML = isLight ? 
        ...
        function _buildFriendCard(f, ci, fi, L, I, groupTags, sortMeta) {
            ...
            <div class="record-header" onclick="toggleFriendRecordOpen(${ci}, ${fi})">
                <div style="display:flex; flex-direction:column; min-width:0;">
                    <div class="friend-name-row" style="display:flex; align-items:center; gap:8px;">
                          <span class="friend-display-name">● ${raidSoEscape(displayName)}</span>
                        ${sortMetaHtml}
                ...
                <div style="display: flex; gap: 8px; margin-bottom: 8px;">
                    ...
                    <div style="flex: 1;">
                        ...
                        <div style="display: flex; gap: 4px; align-items: center;">
                            ...
                            <button type="button" class="btn-secondary id-mini-date-btn" onclick="openMiniDatePicker(${ci}, ${fi}, 'anniversary')" style="padding: 4px 6px; height: 26px; display: flex; align-items: center; justify-content: center; border: 1px solid var(--border-color); border-radius: 4px; cursor: pointer; background: var(--bg-item); color: var(--text-main);" title="${raidSoEscape(I.chooseFromCalendar)}">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                            </button>
                        </div>
                    </div>
...
// Wheel control for record count inputs (Focus-linked throttle)
let lastCountWheelTime = 0;
function handleCountInputWheel(e, ci, ri, inputEl) {
    if (document.activeElement !== inputEl) return;
    e.preventDefault();
    e.stopPropagation();
    const now = Date.now();
    if (now - lastCountWheelTime < 180) return;
    lastCountWheelTime = now;
    const step = e.deltaY < 0 ? 1 : -1;
    stepRecordCount(ci, ri, step);
}
window.handleCountInputWheel = handleCountInputWheel;
