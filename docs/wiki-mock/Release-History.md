...
スクリーンショット、更新手順、更新後の確認項目は[1.0.3リリース案内](Release-1.0.3)を参照してください。
...
- 幅480px以下のドックでカードと操作ボタンを折り返し、幅300px以下ではヘッダーを複数行へ配置するよう調整しました。
- 復元時の入力検証とロールバックを追加し、不正なJSONまたはTXTで既存データが変更されないよう修正しました。
- レイド開始失敗時のAPIエラーとアウトバウンドレイドログが、選択中の表示言語で表示されるよう修正しました。
