...
| 種類 | 例 | 用途 |
| --- | --- | --- |
| 開発版 | `0.9_beta` | `dev_0.9_beta`で修正を集め、公開前に確認する版 |
| 正式版 | `1.0.0` | `main`で公開する最初の正式版 |
| 軽微なバグ修正 | `1.0.1` | 機能を変えない修正版 |
| 機能追加 | `1.1.0` | 後方互換性を保った機能追加版 |

開発版は `X.X_beta` の2桁、正式版は `X.Y.Z` の3桁で管理します。現在の開発統合ブランチは `dev_0.9_beta` です。`uka` の更新をまとめた `0.9_beta` の内容は、`dev_0.9_beta` に反映済みです。正式版1.0.0の公開予定日は2026年7月25日です。
